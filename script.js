...
paymentForm.addEventListener('submit', function(e) {
  ...
  <lottie-player src="https://assets7.lottiefiles.com/packages/lf20_WBZkw7.json" background="transparent" speed="1" style="width: 300px; height: 300px;" loop autoplay></lottie-player>
  `;
  setTimeout(() => {
    document.getElementById("statusMessage").innerHTML = "order received";
  }, 2000);
});
    ...
    <div id="pizza-${cartItem.id}" class="cart-item flex flex-col p-6 border-1 border-b">
     ...
     <p class="font-light text-xs">${cartItem.desc}</p>
      ...
      <div class="cart-item-increment flex flex-auto gap-2 mt-3">
        <button data-minus="${cartItem.id}" id="decreaseAmount" class="w-6 h-6 bg-th-red-500 text-white rounded-full">-
        </button>
        <p class="pizza-amount-${cartItem.id}">${cartItem.amount}</p>  
        <button data-plus="${cartItem.id}" id="increaseAmount" class="w-6 h-6 bg-th-red-500 text-white rounded-full">+
        </button>
      </div>
...
function getMenu() {
  pizzaMenu.forEach(function (pizza) {
    menuList.innerHTML += `
    <li class="sm:w-72 grid-cols-[1fr-minmax(100px,_2fr)_1fr] items-center justify-center border border-[#bdbdbd] mx-4 p-5 gap-5 rounded-1-[180px] rounded-r-[30px] sm:mx-0  sm:grid-cols-1 sm:rounded-r-[0] sm:ronded-l-[0] sm:rounded-t-[180px] sm:rounded-b-[30px] sm:grid-rows-[1fr_minmax(150px,auto)_auto]">
    <img class="w-24 sm:w-72" src="${pizza.img}" alt="" />
    <div>
    <h3 class="font-semibold text-2x1 text-left sm:text-center">${pizza.name}</h3>
    <p class="font-thin leading-[18px] mt-4 text-sm text-left sm:text-center">${pizza.desc}</p> 
    </div>
    <div class="text-center">
   <p class="text-th-green-400 font-semibold text-2x1 leading-8">$${pizza.price}</p>
   <button data-id="${pizza.id}" class="font-medium text-xs w-full mt-2 bg-th-red-500 text-white uppercase px-6 py-2 ronded-3x1">add to cart</button>
    </div>
    </li>
    `;
  });
